...
ممیزی امنیتی وابستگی‌های تولید (Production Dependencies) با استفاده از `npm audit --omit=dev`.

### خلاصه یافته‌ها
- **تعداد کل آسیب‌پذیری‌ها:** ۱۴ مورد
- **تفکیک شدت:** ۵ مورد High، ۹ مورد Moderate، ۰ مورد Critical/Low.
- **وضعیت وابستگی‌ها:** اکثر آسیب‌پذیری‌ها مربوط به نسخه‌های NestJS 10 و Next.js 14 هستند که نیاز به ارتقای Major برای اصلاح قطعی دارند.

### ماتریس آسیب‌پذیری‌های تولید
| پکیج | شدت | Direct/Transitive | نسخه فعلی | نسخه اصلاح‌شده | ریسک سازگاری |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `@nestjs/core` | Moderate | Direct | 10.4.22 | 11.2.1 | **بالا** (Major Jump: NestJS 10 → 11) |
| `@nestjs/platform-express` | High | Direct | 10.4.22 | 11.2.1 | **بالا** (شامل ارتقای express و multer) |
| `next` | High | Direct | 14.2.35 | 16.3.1 | **بالا** (Major Jump: Next.js 14 → 16) |
| `@nestjs/config` | Moderate | Direct | 3.3.0 | 4.0.4 | **بالا** (وابسته به ارتقای NestJS) |
| `@nestjs/common` | Moderate | Direct | 10.4.22 | 10.4.23+ / 11.x | **متوسط** (تاثیر از `file-type`) |
| `express` | Moderate | Direct & Transitive | 4.22.2 / 4.22.1 | 4.22.2+ | **پایین** (اصلاح `qs` و `body-parser`) |
| `lodash` | High | Transitive | 4.17.21 | 4.17.24+ | **پایین** (وصله امنیتی در `@nestjs/config`) |
| `multer` | High | Transitive | 2.0.2 | 2.2.0 | **متوسط** (تغییرات در آپلود فایل) |
| `postcss` | High | Transitive | 8.4.31 | 8.5.23+ | **پایین** (وصله امنیتی در `next`) |
| `body-parser` | Moderate | Transitive | 1.20.4 / 1.20.6 | 1.20.7+ | **پایین** (وصله DoS) |
| `file-type` | Moderate | Transitive | 20.4.1 | 21.3.2+ | **پایین** (وصله DoS) |
| `qs` | Moderate | Transitive | 6.14.2 / 6.15.3 | 6.15.4+ | **پایین** (وصله DoS) |

### تحلیل و پیشنهادها
1. **ارتقای NestJS (High Risk):** ارتقا از نسخه ۱۰ به ۱۱ برای اصلاح `@nestjs/core` و وابستگی‌های مرتبط ضروری است. این اقدام نیازمند تست کامل رگرسیون در `apps/server` به دلیل تغییرات احتمالی در metadata reflection و RxJS است.
2. **ارتقای Next.js (High Risk):** پرش از نسخه ۱۴ به ۱۶ بسیار حساس است. پیشنهاد می‌شود ابتدا به نسخه ۱۵ پایدار ارتقا یابد و پس از تثبیت، به ۱۶ منتقل شود.
3. **وصله‌های گذرا (Safe):** به‌روزرسانی `lodash` و `postcss` از طریق `npm update` یا اصلاح lockfile با کمترین ریسک همراه است و باید در اولویت اول قرار گیرد.
4. **تذکر:** هیچ تغییری در این فاز در `package.json` یا `package-lock.json` اعمال نشده است.
