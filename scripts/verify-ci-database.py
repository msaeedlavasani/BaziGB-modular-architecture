...
try:
    ...
    missing = [model for model, physical in required.items() if physical not in tables]
finally:
    con.close()
with open(db_path, 'rb') as stream:
    digest = hashlib.sha256(stream.read()).hexdigest()
result = {'gate':'DATABASE_CONTRACT','databaseTarget':db_path,'integrity':integrity,'userVersion':version,'requiredModels':required,'missingModels':missing,'physicalTables':sorted(tables),'sha256':digest,'result':'PASS' if integrity == 'ok' and not missing else 'FAIL','nextSafeAction':'continue to runtime readiness' if not missing else 'initialize the exact runtime schema target'}
